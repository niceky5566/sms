        ...
        if (!$error) {
            ...
            try {
                ...
                if (!$db->tableExists($table_category)) {
                    ...
                    $typies = [];
                    $save = [];
                    foreach ($db->search($table_category) as $item) {
                        $topic = @unserialize($item->topic);
                        if (is_array($topic)) {
                            $typies[] = $item->type;
                            foreach ($topic as $lng => $value) {
                                $save[] = array(
                                    'type' => $item->type,
                    ...
                    $db->delete($table_category, array('type' => $typies), 0);
                ...
                if ($db->fieldExists($table_edocument, 'reciever')) {
                    $db->query("ALTER TABLE `$table_edocument` CHANGE `reciever` `receiver` TEXT NOT NULL");
